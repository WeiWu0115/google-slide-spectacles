...
const server = http.createServer((req, res) => {
  ...
  // Serve direct-controller.html
  if (parsedUrl.pathname === '/direct-controller.html' || parsedUrl.pathname === '/direct-controller') {
    try {
      const filePath = path.join(__dirname, 'direct-controller.html');
      const content = fs.readFileSync(filePath, 'utf8');
      res.writeHead(200, { 'Content-Type': 'text/html' });
      res.end(content);
      return;
    } catch (error) {
      console.error('Error serving direct-controller.html:', error);
      res.writeHead(404, { 'Content-Type': 'text/plain' });
      res.end('File not found');
      return;
    }
